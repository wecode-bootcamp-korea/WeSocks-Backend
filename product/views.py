    ...
    def post(self, request):
        ...
        if check_design["check_design_exists"]:
            ...
            return JsonResponse({
                ...
                "design_id"      : check_design["the_design_id"]
    ...
    def post(self, request):
        ...
        if check_wish_existence:
            ...
            new_wish = WishesDetail.objects.create(
                design = DesignDescription.objects.get(id = the_design_id),
                user   = User.objects.get(id = wish_req["user_pk"]),
            )
            res_message  = "success"
            wished_count = DesignDescription.objects.get(id = the_design_id).wished_num
...
class AddCartReqView(View):
    ...
    def post(self, request):
        ...
        if "design_id" in add_cart_req:               
            the_design_id = add_cart_req["design_id"]
        # If front end does not know what the design id is >>
        else: 
            check_design  = check_design_existence(add_cart_req)
            the_design_id = check_design["the_design_id"]

        the_design   = DesignDescription.objects.get(id = the_design_id)
        new_add_cart = CartDetail.objects.create(
            design        = DesignDescription.objects.get(id = the_design_id),
            user          = User.objects.get(id = add_cart_req["user_pk"]),
            count         = add_cart_req["count"],
            total_price   = float(add_cart_req["count"]) * float(the_design.unit_price),
            reward_points = float(add_cart_req["count"]) * float(the_design.unit_price) * 0.1
        )
...
class ChangeCartReqView(View):
    ...
    def post(self, request):
        ...
        if check_cart_flag:
            ...
            the_design         = cart_to_be_changed.design
            cart_to_be_changed.count         = change_cart_req["count"]
            cart_to_be_changed.total_price   = float(change_cart_req["count"]) * float(the_design.unit_price)
            cart_to_be_changed.reward_points = float(change_cart_req["count"]) * float(the_design.unit_price) * 0.1
